...
export async function listBanners(req, res) { const now = new Date(); const banners = await Banner.find({ isActive: true, $or: [{ startsAt: null }, { startsAt: { $lte: now } }] }).sort({ sortOrder: 1, createdAt: -1 }); res.json({ banners }); }
export async function adminCreateBanner(req, res) { const banner = await Banner.create(req.body); res.status(201).json({ banner }); }
export async function adminListBanners(req, res) { const banners = await Banner.find().sort({ sortOrder: 1, createdAt: -1 }); res.json({ banners }); }
